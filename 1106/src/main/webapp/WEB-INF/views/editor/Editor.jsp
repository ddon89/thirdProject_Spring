		...
		<div id = "objects">
			<div id = "objects1">
				...
				<div>
					...
					<!-- 가져온 아이템이 Player일 때 -->
					<c:if test="${list[stat.index].type eq 'Player'}">
						<button data-itemno = "${list[stat.index].no}"
								 data-itemscale = "${list[stat.index].scale}" class ="Player">
							<img src = "down_Icon_img?icon=${loginUser.profile}.png" alt = "${loginUser.profile}">
					...
					<!-- 가져온 아이템이 플레이어가 아닐 때 -->
					<c:if test='${list[stat.index].type ne "Player"}'>
						<button data-itemno = "${list[stat.index].no}"
								 data-itemscale = "${list[stat.index].scale}" 
								 class =${list[stat.index].type eq "floor"? "callFloor" : "callObjBtn"}>
