...
//		this.saveScene(scene);
	...
	//플레이어 카메라 위치 설정
	, addPlayerPosition : function(scene, profile){
		console.log("플레이어 위치 지정하기");
		this.mouseClickFunction(null, scene, "removeSecneGroup");
		group = this.findGroup(scene);
		if(group) scene.remove(group);
		
		MapEdit01.group = new THREE.Group();
		console.log(MapEdit01.group);
		
		//플레이 할 때 시작할 플레이어의 위치를 지정. set_items의 Position 컬럼을 이용
		//플레이어 프로필 로딩
		var loader = new THREE.JSONLoader();
		var pg = new THREE.PlaneGeometry(10,10,1,1);
		var texture = THREE.ImageUtils.loadTexture("resources/images/" + profile);
		...
		var player = new THREE.Mesh(pg, pm);
		player.name = "Player";
		player.position.set(0,5,0);
		
		//화살표 로딩
		var mesh;
		loader.load("resources/json/model/playerArrow.json", function(geom, mat) {
			mesh = new THREE.Mesh(geom, mat[0]);
			mesh.name = "arrows";
			mesh.position.set(0,10,0);
			mesh.scale.set(5,5,5);
			player.add(mesh);
			tempMesh = MapEdit02.addMesh(player);
			MapEdit01.group.add(tempMesh);
			MapEdit01.group.add(player);
		});
    			...
    			if(intersects.length > 0){
    				if(intersects[0].object.geometry instanceof THREE.PlaneGeometry){
    					...
    					if(temp.geometry instanceof THREE.BoxGeometry) {
    						...
    						console.log("플레이어 위치설정");
    						height = new THREE.Box3().setFromObject(temp).center().y;
    						console.log(height);
    						temp.name = "Player";
