...
//		scene = MapEdit01.cancel(scene);
	...
	function firstEditorSetting(){
		...
		$(document).on("click",".Player", function(){
			...
			var profile = $(".Player").children("img").attr("src");
			profile = profile.split("=")[1];
			scene = MapEdit01.addPlayerPosition(scene, profile);
		});
	}
	...
	function render() {
		...
		//player 돌리기
		if(MapEdit01.group){
			find = MapEdit01.group.children.find(function(item){
				return item.name == "Player";
			});
			if(find){
				console.log(find);
				MapEdit01.group.rotation.y += 0.05;
			}
		}
		
		//scene에 위치한 플레이어 회전
		finds = scene.children.find(function(item){
			return item.name == "Player";
		});
		if(finds) {
			finds.children[0].rotation.y +=0.05;
		}
				...
				$.each(resp, function(index, item){
					...
					if(item.setting){
						...
						if(item.item_name == "plane"){
							...
							var texture = THREE.ImageUtils.loadTexture(item.rotation);
							...
							scene.add(player);
							MapEdit01.targetList.push(player);
							MapEdit01.targetGroup.push(player);
