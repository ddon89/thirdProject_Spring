...
public class ItemController {
	...
	public ArrayList<Set_Items> getSavedItems(Set_Items item){
		...
		for(Set_Items i : savedItems){
			System.out.println("GetSaved Item : " + i);
		}
	...
	public int insertTrick(Items item){
		System.out.println(item.getContent() +", "+ item.getMap_no() +", "+ item.getSolved());
		System.out.println("저장하기 위해 받아오는 아이템 : " + item);
		
		Set_Items set = new Set_Items();
		set.setMap_no(item.getMap_no());
		set.setItem_name(item.getItem_name());
		System.out.println(set);
		int si_no = irepo.selecteSetItem(set).get(0).getNo();
		System.out.println(si_no);
		
		item.setSI_no(si_no);
		System.out.println("시리얼 넘버 받은 아이템 : " + item);
